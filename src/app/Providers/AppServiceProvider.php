...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        if (env('ENV_OVERRIDE')) {
            ...
            // From Database — use value() which returns null (not crash) when the row is missing
            \Config::set('laravel-omnipay.gateways.paypal_express.credentials.username', \App\ApiKey::where('key', 'paypal_username')->value('value'));
            \Config::set('laravel-omnipay.gateways.paypal_express.credentials.password', \App\ApiKey::where('key', 'paypal_password')->value('value'));
            \Config::set('laravel-omnipay.gateways.paypal_express.credentials.signature', \App\ApiKey::where('key', 'paypal_signature')->value('value'));
            // Stripe
            \Config::set('laravel-omnipay.gateways.stripe.credentials.public', \App\ApiKey::where('key', 'stripe_public_key')->value('value'));
            \Config::set('laravel-omnipay.gateways.stripe.credentials.secret', \App\ApiKey::where('key', 'stripe_secret_key')->value('value'));
            // Challonge
            \Config::set('challonge.api_key', \App\ApiKey::where('key', 'challonge_api_key')->value('value'));
            // Steam
            \Config::set('steam-auth.api_key', \App\ApiKey::where('key', 'steam_api_key')->value('value'));
        }
